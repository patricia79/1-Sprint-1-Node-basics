...
numeroMayorMenor5().then(resolve => console.log(resolve))
...
funcioAmbParametre = (param1) => {
    console.log(param1);
}

callbackAmbParametre = (callback, parametre) => {
    console.log('Executant la callback amb paràmetre.');
    callback(parametre);
}

callbackAmbParametre(funcioAmbParametre, "Passem un paràmetre.");
...
employee.id = 3;
getEmployee(employee.id).then(id => console.log("n2e1 - Trobat"))
...
employee = new Object();
employee.id = 1;
getSalary(employee).then(salary => console.log("n2e2 - L'Empleat te un salary de: " + salary))
